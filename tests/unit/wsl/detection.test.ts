...
describe('WSL detection utilities', () => {
  ...
  describe('getWindowsHostIpForWsl', () => {
    it('returns 127.0.0.1 when .wslconfig has networkingMode=mirrored', () => {
      restorePlatform = mockPlatform('win32')
      mockReadFileSync.mockReturnValue('[wsl2]\nnetworkingMode=mirrored\n')

      expect(getWindowsHostIpForWsl('Ubuntu')).toBe('127.0.0.1')
    })

    it('returns 127.0.0.1 when .wslconfig has networkingMode = mirrored (with spaces)', () => {
      restorePlatform = mockPlatform('win32')
      mockReadFileSync.mockReturnValue('[wsl2]\nnetworkingMode = mirrored\n')
    ...
    it('returns 127.0.0.1 when .wslconfig has NetworkingMode=mirrored (case insensitive)', () => {
      restorePlatform = mockPlatform('win32')
      mockReadFileSync.mockReturnValue('[wsl2]\nNetworkingMode=mirrored\n')
    ...
    it('returns 127.0.0.1 for WSL1 distros', () => {
      restorePlatform = mockPlatform('win32')
      // .wslconfig unreadable (no mirrored mode)
      mockReadFileSync.mockImplementation(() => { throw new Error('ENOENT') })

      // listWslDistros returns WSL1 distro
      const text = [
        '  NAME                   STATE           VERSION',
        '* Ubuntu                 Running         1',
      ].join('\r\n')
    ...
    it('extracts host IP from /etc/resolv.conf for WSL2 NAT mode', () => {
      restorePlatform = mockPlatform('win32')
      // No .wslconfig
      mockReadFileSync.mockImplementation(() => { throw new Error('ENOENT') })

      // listWslDistros returns WSL2 distro
      const listText = [
        '  NAME                   STATE           VERSION',
        '* Ubuntu                 Running         2',
      ].join('\r\n')
      const listBuf = Buffer.from('\uFEFF' + listText, 'utf16le')
      mockExecSync.mockReturnValue(listBuf)

      // resolv.conf with nameserver
      const resolvConf = '# This file was automatically generated by WSL.\nnameserver 172.25.192.1\n'
      mockExecFileSync.mockReturnValue(Buffer.from(resolvConf, 'utf-8'))

      expect(getWindowsHostIpForWsl('Ubuntu')).toBe('172.25.192.1')
      expect(mockExecFileSync).toHaveBeenCalledWith(
        'wsl.exe',
        ['-d', 'Ubuntu', '--', 'cat', '/etc/resolv.conf'],
        expect.objectContaining({ timeout: expect.any(Number) }),
      )
    })

    it('returns 127.0.0.1 as fallback when resolv.conf has no nameserver', () => {
      restorePlatform = mockPlatform('win32')
      mockReadFileSync.mockImplementation(() => { throw new Error('ENOENT') })
      ...
      mockExecSync.mockReturnValue(Buffer.from('\uFEFF' + listText, 'utf16le'))

      // resolv.conf with no nameserver line
      mockExecFileSync.mockReturnValue(Buffer.from('# empty\n', 'utf-8'))

      expect(getWindowsHostIpForWsl('Ubuntu')).toBe('127.0.0.1')
    })

    it('returns 127.0.0.1 as fallback when all checks fail', () => {
      restorePlatform = mockPlatform('win32')
      mockReadFileSync.mockImplementation(() => { throw new Error('ENOENT') })
      mockExecSync.mockImplementation(() => { throw new Error('fail') })
    ...
    it('does not treat networkingMode=nat as mirrored', () => {
      restorePlatform = mockPlatform('win32')
      mockReadFileSync.mockReturnValue('[wsl2]\nnetworkingMode=nat\n')
      ...
      // resolv.conf
      mockExecFileSync.mockReturnValue(Buffer.from('nameserver 172.25.192.1\n', 'utf-8'))
