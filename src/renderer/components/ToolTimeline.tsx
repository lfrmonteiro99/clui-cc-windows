...
export function getToolIcon(toolName: string, toolInput?: string): Icon {
  // Check for git operations in Bash commands
  ...
  try {
    ...
    switch (toolName) {
      ...
      case 'Bash': {
        ...
        const words = cmd.trim().split(/\s+/)
        return words[0] ? truncate(words[0], 16) : 'Bash'
      }
      case 'Glob':
        return parsed.pattern ? truncate(parsed.pattern, 20) : 'Glob'
      case 'Grep':
        return parsed.pattern ? truncate(parsed.pattern, 20) : 'Grep'
      case 'WebSearch':
        return truncate(parsed.query || parsed.search_query || 'Search', 20)
      case 'WebFetch':
        return 'Fetch'
      case 'Agent':
        return 'Agent'
...
function getToolDuration(tool: Message, index: number, tools: Message[]): string | null {
  // If the tool is still running, no duration yet
  if (tool.toolStatus === 'running') return null

  // Use timestamp difference from this tool to the next one (or end)
  // Tools within a group are sequential, so diff between timestamps is the duration
...
// ─── Tool detail rendering (preserves existing behavior) ───
...
function ToolDetail({ tool }: { tool: Message }) {
  ...
  // Write tool: has file_path, content (all additions)
...
/** Full description (reused from ConversationView logic) */
...
export const ToolTimeline = React.memo(function ToolTimeline({ tools, skipMotion }: ToolTimelineProps) {
  ...
  if (isOpen) {
    const inner = (
      <div className="py-1" data-testid="tool-timeline">
        ...
        {/* Pill strip */}
        <div className="flex flex-wrap gap-1.5 mb-1.5" data-testid="tool-pill-strip">
          {tools.map((tool, idx) => {
            const toolName = tool.toolName || 'Tool'
            const Icon = getToolIcon(toolName, tool.toolInput)
            ...
            return (
              ...
              >
                ...
                {duration && (
                  <span style={{ color: colors.textMuted, fontSize: 10 }}>{duration}</span>
                )}
              </button>
            )
          })}
  ...
  const inner = (
    ...
    >
      <CaretRight size={10} className="flex-shrink-0" style={{ color: colors.textTertiary }} />
      ...
      <span className="flex items-center gap-0.5 ml-0.5">
        {tools.slice(0, 6).map((tool) => {
          const Icon = getToolIcon(tool.toolName || 'Tool', tool.toolInput)
          return (
            <span
              key={tool.id}
